...
val lift_atomic_targeted_post_result_observed_result_hidden_state_action
...
val lift_atomic_observed_result_hidden_state_action_invs
    (#a:Type u#a)
    (#pre:slprop)
    (#post:a -> slprop)
    (#opens:inames)
...
(** Non-diverging active-runner view for the Ret-specialized hidden-state
    observed-result lowering used by public Resolve. *)
...
(** Non-diverging active-runner view for the same Ret-specialized hidden-state
    NewProph lowering.  Adequacy code can use this checked entry point when it
    owns [si ot c], avoiding the ordinary counter-erased [Sem.step] branch for
    hidden-state prophecy actions. *)
val lift_fresh_prophecy_id_hidden_state_action_active_run
    (#a:Type u#a)
    (#post:a -> slprop)
    (#si:NST.obs_tape -> NST.ctr -> slprop)
    (e:(pid:nat -> #ot:erased NST.obs_tape -> #c:erased (c:NST.ctr{pid == NST.prophecy_index c}) ->
