...
val pers_intro_star (p q: slprop) : squash ((pers p `star` pers q) `implies` pers (p `star` q))
...
val pers_exists_sym (#a: Type u#a) (f: a -> slprop) : squash ((exists* x. pers (f x)) == pers (exists* x. f x))
val pers_intro_inv (i: iref) (p: slprop) : squash (inv i p `implies` pers (inv i p))
val pers_intro_slprop_ref (x: slprop_ref) (p: slprop) : squash (slprop_ref_pts_to x p `implies` pers (slprop_ref_pts_to x p))