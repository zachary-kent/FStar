...
let au_inv_p (is:inames) (a:Type0) (b:Type0) (alpha : a -> slprop) (beta : a -> b -> slprop)
  ...
  exists* (flag:bool). pts_to gr #0.5R flag **
    (if flag then
       (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y)))
     else emp)
...
ghost fn fold_au_inv_p (is:inames) (a:Type0) (b:Type0) (alpha : a -> slprop) (beta : a -> b -> slprop)
  ...
  requires pts_to gr #0.5R flag **
    (if flag then (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y))) else emp)
...
let au_opened (#is:inames) (#a:Type0) (#b:Type0)
  ...
  (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y))
  ...
  ensures au_available tok
{
  ...
  GR.share gr;
  fold (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y)));
  rewrite (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y)))
       as (if true then (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y))) else emp);
  ...
  ensures alpha (reveal x) ** au_opened tok (reveal x)
{
  ...
  with_invariants_g unit emp_inames tok.i (au_inv_p is a b alpha beta phi tok.gr)
    ...
    (fun _ -> (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y))) ** pts_to tok.gr #0.5R false)
  fn _ {
    unfold au_inv_p;
    with bv. assert (pts_to tok.gr #0.5R bv ** pts_to tok.gr #0.5R true);
    GR.gather tok.gr #true #_;
    rewrite each bv as true;
    rewrite (if true then (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y))) else emp)
         as (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y)));
    tok.gr := false;
    GR.share tok.gr;
    rewrite emp as (if false then (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y))) else emp);
    fold_au_inv_p is a b alpha beta phi tok.gr;
  };
  // Unpack: ∃x. alpha x ** forall* y. trade
  let x = elim_exists #a (fun (x:a) -> alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y)));
  ...
  ensures au_available tok
{
  ...
  fold (exists* (xx:a). alpha xx ** (forall* (y:b). trade #is (later_credit 1 ** beta xx y) (phi xx y)));
  with_invariants_g unit emp_inames tok.i (au_inv_p is a b alpha beta phi tok.gr)
    ((exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y))) ** pts_to tok.gr #0.5R false)
  ...
  fn _ {
    ...
    rewrite (if false then (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y))) else emp) as emp;
    drop_ emp;
    tok.gr := true;
    GR.share tok.gr;
    rewrite (exists* (x:a). alpha x ** (forall* (y:b). trade #is (later_credit 1 ** beta x y) (phi x y)))
  ...
  ensures phi (reveal x) y
{
  ...
  elim_forall y;
  elim_trade #is (later_credit 1 ** beta (reveal x) y) (phi (reveal x) y);
