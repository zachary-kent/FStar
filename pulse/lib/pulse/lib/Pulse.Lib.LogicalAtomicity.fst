...
let au_inv_p (a:Type0) (b:Type0) (alpha : a -> slprop) (beta : a -> b -> slprop)
    (phi : a -> b -> slprop) (gr : GR.ref bool) : slprop =
  exists* (flag:bool). pts_to gr #0.5R flag **
    (if flag then
       (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y))
     else emp)

ghost fn fold_au_inv_p (a:Type0) (b:Type0) (alpha : a -> slprop) (beta : a -> b -> slprop)
    (phi : a -> b -> slprop) (gr : GR.ref bool) (#flag:bool)
  requires pts_to gr #0.5R flag **
    (if flag then (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y)) else emp)
  ensures au_inv_p a b alpha beta phi gr
{ fold (au_inv_p a b alpha beta phi gr) }
...
let au_available (#a:Type0) (#b:Type0)
  ...
  pts_to tok.gr #0.5R true ** inv tok.i (au_inv_p a b alpha beta phi tok.gr)
...
let au_opened (#a:Type0) (#b:Type0)
  ...
  inv tok.i (au_inv_p a b alpha beta phi tok.gr) **
  ...
  ensures au_available tok
{
  ...
  GR.share gr;
  fold (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y));
  rewrite (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y))
       as (if true then (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y)) else emp);
  fold_au_inv_p a b alpha beta phi gr;
  let i = new_invariant (au_inv_p a b alpha beta phi gr);
  let tok : au_token a b alpha beta phi = { gr; i };
  rewrite (pts_to gr #0.5R true) as (pts_to tok.gr #0.5R true);
  rewrite (inv i (au_inv_p a b alpha beta phi gr))
       as (inv tok.i (au_inv_p a b alpha beta phi tok.gr));
  ...
  ensures alpha (reveal x) ** au_opened tok (reveal x)
{
  open GR;
  unfold au_available;
  with_invariants_g unit emp_inames tok.i (au_inv_p a b alpha beta phi tok.gr)
    (pts_to tok.gr #0.5R true)
    (fun _ -> (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y)) ** pts_to tok.gr #0.5R false)
  fn _ {
    unfold au_inv_p;
    with bv. assert (pts_to tok.gr #0.5R bv ** pts_to tok.gr #0.5R true);
    GR.gather tok.gr #true #_;
    rewrite each bv as true;
    rewrite (if true then (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y)) else emp)
         as (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y));
    tok.gr := false;
    GR.share tok.gr;
    rewrite emp as (if false then (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y)) else emp);
    fold_au_inv_p a b alpha beta phi tok.gr;
  };
  // Unpack: ∃x. alpha x ** forall* y. trade
  let x = elim_exists #a (fun (x:a) -> alpha x ** (forall* (y:b). beta x y @==> phi x y));
  ...
  ensures au_available tok
{
  ...
  fold (exists* (xx:a). alpha xx ** (forall* (y:b). beta xx y @==> phi xx y));
  with_invariants_g unit emp_inames tok.i (au_inv_p a b alpha beta phi tok.gr)
    ((exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y)) ** pts_to tok.gr #0.5R false)
  ...
  fn _ {
    ...
    rewrite (if false then (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y)) else emp) as emp;
    drop_ emp;
    tok.gr := true;
    GR.share tok.gr;
    rewrite (exists* (x:a). alpha x ** (forall* (y:b). beta x y @==> phi x y))
  ...
  ensures phi (reveal x) y
{
  ...
  elim_forall y;
  elim_trade (beta (reveal x) y) (phi (reveal x) y);
  // Drop the AU remnants
  drop_ (pts_to tok.gr #0.5R false);
  drop_ (inv tok.i (au_inv_p a b alpha beta phi tok.gr))
}
